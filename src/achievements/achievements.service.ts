...
export class AchievementsService {
  ...
  updateNotification(userId: number, achvId: number) {
  ...
  async getNotifications(userId: number) {
    ...
    const achievements = await Promise.all(
      achvs.map((id: number) => this.achievementsRepository.getAchievement(id)),
    );
  ...
  async unlockAchievement(id: number, userId: number) {
    await this.achievementsRepository.unlock(userId, id);
